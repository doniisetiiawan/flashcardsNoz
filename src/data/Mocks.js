...
const MockReviews = mkReviews(MockCards);
const MockDecks = [
...
MockDecks.map((deck) => {
  deck.addCard(
    new CardModel('der Hund', 'the dog', deck.id),
  );
  deck.addCard(
    new CardModel('die Katze', 'the cat', deck.id),
  );
  deck.addCard(
    new CardModel('das Brot', 'the bread', deck.id),
  );
  deck.addCard(
    new CardModel('die Frau', 'the woman', deck.id),
  );
  return deck;
});
